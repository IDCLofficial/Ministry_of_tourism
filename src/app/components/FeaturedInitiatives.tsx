    ...
    <section className="w-full p-4 md:p-8 py-10 md:py-16 bg-white flex flex-col items-center gap-8">
      ...
      <div className="w-full mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 px-0 md:px-4">
        {projects?.slice(0, 4).map((project) => (
          <div
            key={project.title} 
            className="bg-transparent relative cursor-pointer hover:scale-105 transition-all duration-300"
          >
            <motion.div 
              initial={{opacity: 0, y:80}}
              whileInView = {{opacity: 1, y:0}}
              transition={{duration:1, ease:"easeInOut"}}   
              className="bg-white h-full flex flex-col items-center shadow-md border-gray-200"  
            >
